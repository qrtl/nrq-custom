...
class HrAttendance(models.Model):
    ...
    update_manually = fields.Boolean(string="Updated Manually", default=False,)
    ...
    state = fields.Selection(related="sheet_id.state", readonly=True,)

    @api.constrains("update_manually", "update_manually_reason")
    def _check_update_manually(self):
        if self.update_manually and (
            not self.update_manually_reason
            or self.update_manually_reason == "other"
            and not self.update_manually_reason_desc
        ):
    ...
    def _search_sheet(self, operator, value):
        ...
        for ts in self.env["hr_timesheet_sheet.sheet"].browse(value):
            ...
            local_date_from_dt = local_tz.localize(
                datetime.strptime(ts.date_from, DEFAULT_SERVER_DATE_FORMAT)
            ).replace(hour=0, minute=0, second=0)
            local_date_to_dt = local_tz.localize(
                datetime.strptime(ts.date_to, DEFAULT_SERVER_DATE_FORMAT)
            ).replace(hour=23, minute=59, second=59)
            utc_date_from_dt = local_date_from_dt.astimezone(pytz.utc).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
