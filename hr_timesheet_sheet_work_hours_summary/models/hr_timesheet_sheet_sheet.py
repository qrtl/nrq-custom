...
class HrTimesheetSheet(models.Model):
    ...
    standard_work_hours = fields.Float(
        compute="_compute_standard_work_hours", string="Standard Working Hours",
    )
    expected_work_hours = fields.Float(
        compute="_compute_expected_work_hours", string="Expected Working Hours",
    )
    ...
    overtime_hours = fields.Float(
        compute="_compute_overtime_hours", string="Overtime Working Hours",
    )
    holiday_hours = fields.Float(
        compute="_compute_holiday_hours", string="Holiday Hours",
    )
    ...
    def _compute_timesheet_expected_work_hours(self):
        for sheet in self:
            sheet.timesheet_expected_work_hours = sheet.get_standard_work_hours(
                sheet.date_to
    ...
    def get_holiday_hours(self, date):
        ...
        public_holiday_line_ids = self.env["hr.holidays.public.line"].search(
            [("date", ">=", self.date_from), ("date", "<=", date_to)]
        )
        holiday_hours = 0.0
        for line in public_holiday_line_ids:
            ...
            attendance_ids = self.employee_id.calendar_id.attendance_ids.filtered(
                lambda attendance: int(attendance.dayofweek) == public_holiday.weekday()
            )
            for attendance in attendance_ids:
                holiday_hours += attendance.hour_to - attendance.hour_from
    ...
    def get_standard_work_hours(self, date):
        ...
        date_from = datetime.strptime(
            str(self.date_from), DEFAULT_SERVER_DATE_FORMAT
        ).date()
        date_to = datetime.strptime(
            str(self.date_to), DEFAULT_SERVER_DATE_FORMAT
        ).date()
        day_count = (
            (date_to - date_from).days if date_to < date else (date - date_from).days
        )
        total_time = 0.0
        for single_date in (date_from + timedelta(n) for n in range(day_count + 1)):
            attendance_ids = self.employee_id.calendar_id.attendance_ids.filtered(
                lambda attendance: int(attendance.dayofweek) == single_date.weekday()
            )
            for attendance in attendance_ids:
