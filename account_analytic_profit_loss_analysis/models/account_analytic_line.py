...
class AccountAnalyticLine(models.Model):
    ...
    related_analytic_type = fields.Selection(
        related="analytic_type_id.type", readonly=True,
    )
    ...
    team_id = fields.Many2one(related="sale_id.team_id", store=True, readonly=True,)
    sale_user_id = fields.Many2one(
        related="sale_id.user_id", store=True, readonly=True,
    )
    ...
    amount_int = fields.Integer(
        "Amount", compute="_compute_amount_int", store=True, readonly=True,
    )
    ...
    def _compute_pj_id(self):
        for ln in self:
            ...
            sale_orders = (
                ...
                .sudo()
                .search(
                    [("project_id", "=", ln.account_id.id), ("state", "!=", "cancel")]
                )
            )
    ...
    def create(self, vals):
        ...
        if vals.get("general_account_id") and not vals.get("analytic_type_id"):
            account = self.env["account.account"].browse(vals["general_account_id"])
            analytic_type = account.analytic_type_id
        # assumption: project_id is included only for timesheet entries
        if vals.get("project_id"):
            analytic_type = (
                self.env["analytic.type"].sudo().search([("type", "=", "labour")])[0]
            )
            ...
            if move_line_id:
                ...
                if invoice and invoice.type in ["in_invoice", "in_refund"]:
                    invoice_lines = invoice.invoice_line_ids.filtered(
                        lambda r: r.account_analytic_id.id == vals["account_id"]
                    )
                    purchase_line = (
                        invoice_lines[0].mapped("purchase_line_id")
                        if invoice_lines
                        else False
                    )
                    if purchase_line:
