...
class HrPrivateInfo(models.Model):
    ...
    temp_save = fields.Boolean("Temporary Save",)
    family_name = fields.Char(related="employee_id.family_name", store=True,)
    given_name = fields.Char(related="employee_id.given_name", store=True,)
    furi_family_name = fields.Char(related="employee_id.furi_family_name", store=True,)
    furi_given_name = fields.Char(related="employee_id.furi_given_name", store=True,)
    ...
    private_country_id = fields.Many2one("res.country", string="Nationality (Private)",)
    private_country_code = fields.Char(related="private_country_id.code",)
    roman_family_name = fields.Char("Family Name (Roman)",)
    roman_given_name = fields.Char("Given Name (Roman)",)
    birthday = fields.Date("Birthday",)
    gender = fields.Selection([("male", "Male"), ("female", "Female")],)
    private_phone = fields.Char("Private Phone",)
    private_email = fields.Char("Private Email",)
    postal_code = fields.Char("Postal Code",)
    address_pref = fields.Char("Prefecture", placeholder="Prefecture",)
    address_street = fields.Char("Current Address",)
    building = fields.Char("Apartment/Building",)
    furi_address = fields.Char("Address Furigana",)
    furi_building = fields.Char("Apartment/Building Furigana",)
    # we will not use ir.attachment to store PDF for security reason
    residence_cert = fields.Binary(string="Residence Certificate",)
    residence_cert_filename = fields.Char(string="Residence Cert File Name",)
    ...
    emerg_contact_desc = fields.Char("Emerg. Contact Description",)
    emerg_contact_name = fields.Char("Emerg. Contact Name",)
    emerg_contact_postal_code = fields.Char("Emerg. Contact Postal Code",)
    emerg_contact_address = fields.Char("Emerg. Contact Address",)
    emerg_contact_phone = fields.Char("Emerg. Contact Phone",)
    ...
    furi_bank_acc_holder = fields.Char("Account Holder Furigana",)
    school_name = fields.Char("School Name",)
    school_dept_name = fields.Char("Deartment/Course Name",)
    school_completion = fields.Selection(
        [("completed", "Completed"), ("unfinished", "Unfinished"), ("other", "Other")],
        "School Completion",
    )
    ...
    pension_code = fields.Char(
        "Pension Number Code",
        help="Please input '0000-000000' in case you are not sure about the " "number.",
    )
    pension_seq = fields.Char("Pension Number Sequence",)
    pension_number = fields.Char(
        "Pension Number", compute="_compute_pension_number", store=True,
    )
    ...
    emp_ins_number_2nd = fields.Char("Emp. Ins. Number (2nd part)",)
    emp_ins_number_3rd = fields.Char("Emp. Ins. Number (3rd part)",)
    emp_ins_number = fields.Char(
        "Emp. Insurance Number", compute="_compute_emp_ins_number", store=True,
    )
    note = fields.Text()
    visa_number = fields.Char("Visa Number",)
    date_visa_expiry = fields.Date("Visa Expiry Date",)
    residence_card = fields.Binary("Residence Card",)
    residence_card_filename = fields.Char("Residence Card File Name",)
    ...
    def _onchange_family_name(self):
        if self.family_name:
            self.family_name = jaconv.h2z(self.family_name, ascii=True, digit=True)

    @api.onchange("given_name")
    def _onchange_given_name(self):
        if self.given_name:
            self.given_name = jaconv.h2z(self.given_name, ascii=True, digit=True)

    @api.onchange("furi_family_name")
    def _onchange_furi_family_name(self):
        if self.furi_family_name:
            self.furi_family_name = jaconv.z2h(jaconv.hira2kata(self.furi_family_name))

    @api.onchange("furi_given_name")
    def _onchange_furi_given_name(self):
        if self.furi_given_name:
            self.furi_given_name = jaconv.z2h(jaconv.hira2kata(self.furi_given_name))

    @api.onchange("roman_family_name")
    def _onchange_roman_family_name(self):
        if self.roman_family_name:
            self.roman_family_name, msg = self.check_alphabets(self.roman_family_name)
    ...
    def _onchange_roman_given_name(self):
        if self.roman_given_name:
            self.roman_given_name, msg = self.check_alphabets(self.roman_given_name)
    ...
    def _onchange_private_email(self):
        if self.private_email:
            self.private_email = jaconv.z2h(self.private_email, ascii=True, digit=True)

    @api.onchange("emerg_contact_phone")
    def _onchange_emerg_contact_phone(self):
        if self.emerg_contact_phone:
            self.emerg_contact_phone, msg = self.check_digits(self.emerg_contact_phone)
    ...
    def _onchange_address_pref(self):
        if self.address_pref:
            self.address_pref = jaconv.h2z(self.address_pref, ascii=True, digit=True)
    ...
    def _onchange_furi_bank_acc_holder(self):
        if self.furi_bank_acc_holder:
            ...
            self.furi_bank_acc_holder = "".join(
                jaconv.z2h(jaconv.hira2kata(self.furi_bank_acc_holder)).split()
            )
    ...
    def _onchange_emp_ins_number_1st(self):
        if self.emp_ins_number_1st:
            self.emp_ins_number_1st, msg = self.check_digits(self.emp_ins_number_1st)
    ...
    def _onchange_emp_ins_number_2nd(self):
        if self.emp_ins_number_2nd:
            self.emp_ins_number_2nd, msg = self.check_digits(self.emp_ins_number_2nd)
    ...
    def _onchange_emp_ins_number_3rd(self):
        if self.emp_ins_number_3rd:
            self.emp_ins_number_3rd, msg = self.check_digits(self.emp_ins_number_3rd)
    ...
    def _compute_pension_number(self):
        for rec in self:
            rec.pension_number = (
                "%s" % (rec.pension_code or "") + "-" + "%s" % (rec.pension_seq or "")
            )

    @api.multi
    @api.depends("emp_ins_number_1st", "emp_ins_number_2nd", "emp_ins_number_3rd")
    ...
    def _validate_digit_fields(self):
        for rec in self:
            ...
            if rec.private_phone and not rec.private_phone.encode("utf-8").isdigit():
                raise ValidationError(msg % _("Private Phone"))
            if (
                rec.emerg_contact_phone
                and not rec.emerg_contact_phone.encode("utf-8").isdigit()
            ):
                raise ValidationError(msg % _("Emerg. Contact Phone"))
            if rec.postal_code and not rec.postal_code.encode("utf-8").isdigit():
                raise ValidationError(msg % _("Postal Code"))
            if (
                rec.emerg_contact_postal_code
                and not rec.emerg_contact_postal_code.encode("utf-8").isdigit()
            ):
                raise ValidationError(msg % _("Emerg. Contact Postal Code"))
            if (
                rec.bank_acc_number
                and not rec.bank_acc_number.encode("utf-8").isdigit()
            ):
                raise ValidationError(msg % _("Account Number"))
            if (
                rec.pension_code
                and not rec.pension_code.encode("utf-8").isdigit()
                or rec.pension_seq
                and not rec.pension_seq.encode("utf-8").isdigit()
            ):
                raise ValidationError(msg % _("Pension Number"))
            if (
                rec.emp_ins_number_1st
                and not rec.emp_ins_number_1st.encode("utf-8").isdigit()
                or rec.emp_ins_number_2nd
                and not rec.emp_ins_number_2nd.encode("utf-8").isdigit()
                or rec.emp_ins_number_3rd
                and not rec.emp_ins_number_3rd.encode("utf-8").isdigit()
            ):
    ...
    def _validate_digit_length(self):
        for rec in self:
            ...
            if (
                rec.emerg_contact_postal_code
                and not len(rec.emerg_contact_postal_code) == 7
            ):
                raise ValidationError(msg % (_("Emerg. Contact Postal Code"), "7"))
            ...
            if rec.pension_code and not len(rec.pension_code) == 4:
                raise ValidationError(
                    msg % (_("The first section of Pension Number"), "4")
                )
            if rec.pension_seq and not len(rec.pension_seq) == 6:
                raise ValidationError(
                    msg % (_("The second section of Pension Number"), "6")
                )
            if rec.emp_ins_number_1st and not len(rec.emp_ins_number_1st) == 4:
                raise ValidationError(
                    msg % (_("The first section of Employment Insurance Number"), "4")
                )
            if rec.emp_ins_number_2nd and not len(rec.emp_ins_number_2nd) == 6:
                raise ValidationError(
                    msg % (_("The second section of Employment Insurance Number"), "6")
                )
            if rec.emp_ins_number_3rd and not len(rec.emp_ins_number_3rd) == 1:
                raise ValidationError(
                    msg % (_("The third section of Employment Insurance Number"), "1")
                )
    ...
    def _check_email(self):
        for rec in self:
            ...
            if rec.private_email and not re.match(match, rec.private_email):
    ...
    def create(self, vals):
        ...
        users = self.env.ref("l10n_jp_hr_employee.group_employee_private_info").users
        if users:
    ...
    def action_ready_notification_email_send(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                "l10n_jp_hr_employee", "private_info_submit_notification_email"
            )[1]
        except ValueError:
            template_id = False
        try:
