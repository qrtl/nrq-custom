...
class HrDependant(models.Model):
    ...
    def _default_currency(self):
        ...
        try:
            currency_id = Currency.get_object_reference("base", "JPY")[1]
        except Exception:
            currency_recs = Currency.search([("name", "like", _("JPY"))])
            currency_id = currency_recs[0].id if currency_recs else False
        return currency_id

    name = fields.Char(string="Dependant Name", required=True,)
    furi_name = fields.Char(string="Dependant Name (Furi)", required=True,)
    ...
    name_furigana = fields.Char("Name Furigana",)
    ...
    residence_categ = fields.Selection(
        [("together", "Living Together"), ("separate", "Living Separately")],
        "Residence Category",
        required=True,
    )
    ...
    occupation_desc = fields.Char("Occupation Description",)
    currency_id = fields.Many2one(
        "res.currency", string="Currency", default=_default_currency,
    )
    income = fields.Monetary(
        help="Expected income for the coming year.", required=True,
    )
    amt_to_family = fields.Monetary("Amount Sent to Family",)
    ...
    disability_class = fields.Selection(
        [("normal", "Normal"), ("special", "Special")], string="Disability Class",
    )
    disability_note = fields.Char("Disability Note",)
    is_dependant_tax = fields.Boolean("Dependant in Tax Calc.",)
    ...
    cause_dependant_enter_note = fields.Char("Cause Note",)
    widowhood = fields.Selection(
        [("widow", "Widow"), ("special", "Special Widow"), ("widower", "Widower")],
        help="Input if applicable",
    )
    working_student_deduction = fields.Boolean(
        "Working Student Deduction", help="Input if applicable",
    )
    inactive = fields.Boolean("Inactive", default=False,)
    inactive_date = fields.Date("Inactive Date",)
    inactive_reason = fields.Char("Inactive Reason",)
    appointment_letter_doc = fields.Binary("Letter of Appointment",)
    ...
    def _onchange_phone(self):
        if self.phone:
            self.phone, msg = self.env["hr.private.info"].check_digits(self.phone)
            if not self.phone:
                return msg
