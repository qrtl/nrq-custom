...
class HrEmployee(models.Model):
    ...
    given_name = fields.Char("Given Name",)
    furi_family_name = fields.Char("Family Name Furigana",)
    furi_given_name = fields.Char("Given Name Furigana",)
    employment_type_id = fields.Many2one(
        "hr.employment.type", string="Employment Type",
    )
    private_info_count = fields.Integer(
        compute="_compute_private_info_count", string="Private Info"
    )
    ...
    def action_view_private_info(self):
        ...
        action = self.env.ref(
            "l10n_jp_hr_employee.act_hr_employee_2_hr_private_info"
        ).read()[0]
        if len(private_info) > 1:
            ...
            action["views"] = [
                (
                    self.env.ref("l10n_jp_hr_employee.view_hr_private_info_form").id,
                    "form",
                )
            ]
            if len(private_info) == 1:
